...
//  AFNetworking
//
//  Created by dingkan on 2019/5/6.
//
...
/**
 裁剪按钮代理事件
 */
@protocol WDZDoodleClipViewDelegate <NSObject>
- (void)WDZImageViewFinish:(UIImage *)cropImage; //完成裁剪
- (void)WDZImageViewFinish:(UIImage *)cropImage frame:(CGRect)frame; //完成裁剪
- (void)WDZImageViewCancel;      //取消裁剪
@end

@interface WDZDoodleClipView : UIView
...
/* ----------------- 代理事件--------------------------- */
//裁剪按钮delegate
@property (nonatomic, assign)id<WDZDoodleClipViewDelegate>delegate;

@property (nonatomic, strong) WDZDoodleBoardClipConfigModel *configModel;

- (instancetype)initWithFrame:(CGRect)frame configModel:(WDZDoodleBoardClipConfigModel *)configModel;
