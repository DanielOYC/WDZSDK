 ...
 *  @return 返回转换后的所有拼音
 */
- (NSString *)chineseTransferToPinYin;
...
/**
 必须包含数字和字母
 */
- (BOOL)judgePasswordContainsNumAndLetter;
// 去除首位的空格 和回车
-(NSString *)trimrimmingWhitespaceAndNewlineCharacterSet;
// 判断密码强度
//声明：包含大写/小写/数字/特殊字符
//
//两种以下密码强度低
...
// 匹配mac地址，准确返回mac地址， 错误返回nil
- (NSString *)getDeviceMacString;
...
- (NSMutableAttributedString *)buildAttributeStringWithRangeStr:(NSString *)rangeStr andAttribute:(NSDictionary *)attr;
